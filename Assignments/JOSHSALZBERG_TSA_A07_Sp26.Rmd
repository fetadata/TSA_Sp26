...
libraries <- c('forecast','tseries', 'here', 'dplyr')
...
```{r}
gas_decomp <- decompose(gas_ts, type= "additive")
acf_decomp <- autoplot(Acf(gas_decomp$random, lag.max = 48))
pacf_decomp <- autoplot(Pacf(gas_decomp$random, lag.max = 48))

cowplot::plot_grid(acf_base, acf_decomp, pacf_base,pacf_decomp, ncol = 2, nrow = 2)

gas_seasadj_decomp <- seasadj(gas_decomp)
autoplot(Acf(gas_seasadj_decomp, lag.max = 48))
autoplot(Pacf(gas_seasadj_decomp, lag.max = 48))
```
